...
from app.models.all import Company, Production, Shipment, Inventory, Contract, Loan, Deposit, Mission
from app.services.money import add_money
from app.gamedata import RECIPES, ITEMS
def inv_add(db, uid, item, qty, cost=0.0):
    inv = db.query(Inventory).filter_by(user_id=uid, item_id=item).first()
    if not inv:
        inv = Inventory(user_id=uid, item_id=item, qty=0, avg_cost=0)
        db.add(inv); db.flush()
...
def settle(db: Session, uid: int):
    ...
    for p in db.query(Production).filter_by(user_id=uid, done=False).all():
        if p.end <= now:
            p.done = True
            inv_add(db, uid, p.recipe, p.qty, 0.0)
            rep["produced"].append(f"{p.qty:g}x {p.recipe}")
    ...
    for s in db.query(Shipment).filter_by(user_id=uid, done=False).all():
        if s.arrive <= now:
            s.done = True
            inv_add(db, uid, s.item_id, s.qty, 0.0)
